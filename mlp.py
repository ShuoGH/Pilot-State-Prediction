...
class MLPNetwork_3hidden(nn.Module):
    '''
    Parameters:
        n_features: the input features number.
        n_neurons: [x1,x2,x3] Three hidden layers and the number of neurons in each layer.
    Return:
        softmax probability.
    '''
    ...
    def __init__(self, n_features, n_neurons, dropouts):
        ...
        self.layer3 = nn.Linear(
            in_features=n_neurons[1], out_features=n_neurons[2])
        self.dropout3 = nn.Dropout(dropouts[2])
        self.out_layer = nn.Linear(in_features=n_neurons[2], out_features=4)
